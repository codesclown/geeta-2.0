...
export default function BookViewer({ chapters }: BookViewerProps) {
  ...
  const totalPages = chapters.length * 3 + 2; // cover + 3 pages/chapter + back cover

  // Stop audio when page is hidden (tab switch / phone lock)
  useEffect(() => {
    const onVisibilityChange = () => {
      if (document.hidden && flipAudio) {
        flipAudio.pause();
        flipAudio.currentTime = 0;
      }
    };
  ...
  // Fullscreen change listener — force re-measure after browser completes transition
  useEffect(() => {
    const onChange = () => {
      ...
      setTimeout(() => {
        ...
        const ctrlH = controlsRef.current?.getBoundingClientRect().height ?? 80;
        const maxW = document.fullscreenElement ? 700 : 560;
        if (isDesktop) {
          const availW = rect.width - 110 - 32; // minus sidebar + padding
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      if (isDesktop) {
        ...
        // Mobile: usePortrait=true — one page at a time, full width
  ...
  const goToPage = (page: number) => bookRef.current?.pageFlip()?.flip(page);
  ...
  const goToBookmark = () => {
    ...
    // Small delay ensures flipbook is ready
  ...
  const handleReadAloud = useCallback(() => {
    ...
    const chapterIndex = Math.floor((currentPage - 1) / 3);
    const chapter = chapters[chapterIndex];
    if (!chapter) return;
    const text = lang === "hi"
      ? `अध्याय ${chapter.chapter_number}. ${chapter.name}. ${chapter.summary.hi}`
      : `Chapter ${chapter.chapter_number}. ${chapter.translation}. ${chapter.meaning.en}. ${chapter.summary.en}`;
  ...
  return (
    ...
    >
      ...
      <div className="md:hidden w-full h-full flex flex-col px-3 pt-2 pb-2" style={{ gap: 8 }}>
        ...
        <div className="flex-1 min-h-0 overflow-hidden" style={{ fontSize: `${fontSizeMap[fontSize]}em` }}>
          <HTMLFlipBook
            key="mobile-flipbook"
            ref={bookRef}
            ...
            style={{ width: "100%", height: "100%" }} className=""
            startZIndex={0} autoSize={true}
            clickEventForward={true} useMouseEvents={true}
            swipeDistance={30} showPageCorners={true}
            disableFlipByClick={false} maxShadowOpacity={0.5}
          >
            <CoverPage />
            {chapters.flatMap((chapter) => [
              ...
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              <ChapterPage key={`${chapter.chapter_number}-v`} chapter={chapter} lang={lang} pageType="verses" fontSize={fontSize} ref={null as any} />,
            ])}
            <BackCoverPage totalChapters={chapters.length} />
          </HTMLFlipBook>
        </div>

        <div ref={mobileControlsRef} className="shrink-0 w-full">
          <BookControls currentPage={currentPage} totalPages={totalPages}
            onPrev={goToPrev} onNext={goToNext} onGoToPage={goToPage}
            onReadAloud={handleReadAloud} isReading={isReading} chapters={chapters} isMobile={true} />
        </div>
      </div>
      {/* ── Desktop layout: sidebar + book ── */}
      <div className="hidden md:flex w-full h-full">
        {/* Left sidebar */}
        <div
          ref={langRef}
          className="shrink-0 flex flex-col items-center justify-center gap-4 py-6 px-3"
        ...
        >
          ...
          <div className="flex flex-col gap-1 w-full">
            ...
            {(["hi", "en"] as const).map((l) => (
              <button key={l} onClick={() => setLang(l)}
                className={`w-full py-1.5 rounded-full text-xs font-serif transition-all ${
                  lang === l ? "bg-amber-700 text-amber-100 shadow" : "text-amber-500 hover:text-amber-300 border border-amber-700/30"
                }`}>
                {l === "hi" ? "हिंदी" : "English"}
              </button>
            ))}
          ...
          <div className="flex flex-col gap-1 w-full">
            ...
            {(["sm", "md", "lg"] as FontSize[]).map((s) => (
              <button key={s} onClick={() => setFontSize(s)}
                className={`w-full py-1.5 rounded-full text-xs font-serif transition-all ${
                  fontSize === s ? "bg-amber-700 text-amber-100 shadow" : "text-amber-500 hover:text-amber-300 border border-amber-700/30"
                }`}>
          ...
          <div className="flex flex-col gap-1 w-full">
            ...
            <button onClick={saveBookmark}
              className={`w-full py-1.5 rounded-full text-sm transition-all flex items-center justify-center ${
                bookmarkFlash ? "bg-amber-500 text-amber-950 scale-105" : "text-amber-500 hover:text-amber-300 border border-amber-700/30"
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3 py-2 overflow-hidden">
          ...
          {dims && (
            <div ref={toolbarRef} className="shrink-0 relative mx-auto" style={{ width: bookTotalW, height: dims.h, fontSize: `${fontSizeMap[fontSize]}em` }}>
              <HTMLFlipBook key={`${dims.w}-${dims.h}`} ref={bookRef} width={dims.w} height={dims.h} size="fixed"
              ...
              showCover={true} mobileScrollSupport={true} onFlip={handlePageFlip} style={{}} className=""
              startZIndex={0} autoSize={false} clickEventForward={true} useMouseEvents={true}
          ...
          <div ref={controlsRef} className="shrink-0 w-full max-w-2xl">
            <BookControls currentPage={currentPage} totalPages={totalPages} onPrev={goToPrev} onNext={goToNext}
              onGoToPage={goToPage} onReadAloud={handleReadAloud} isReading={isReading} chapters={chapters} isMobile={false} />
          </div>
        </div>
      </div>
    </div>
...
interface MobileSettingsSheetProps {
  ...
  bookmarkFlash: boolean;
  toggleFullscreen: () => void;
  isFullscreen: boolean;
}

function MobileSettingsSheet({
  lang, setLang, fontSize, setFontSize,
  saveBookmark, goToBookmark, bookmark, bookmarkFlash,
  ...
  return (
    <>
      {/* Invisible refs for height measurement on mobile */}
      ...
      {/* Bottom sheet */}
      <div
        className="md:hidden fixed left-0 right-0 z-50 transition-transform duration-300"
        style={{
          bottom: 0,
          transform: open ? "translateY(0)" : "translateY(100%)",
          background: "linear-gradient(to top, #3b1a08, #4a2010)",
      ...
      >
          ...
          {(["hi", "en"] as const).map((l) => (
            <button
              ...
              className={`flex-1 py-2 rounded-full text-sm font-serif transition-all ${
                lang === l ? "bg-amber-700 text-amber-100 shadow" : "bg-amber-900/60 text-amber-500 border border-amber-700/40"
              }`}
            >
              {l === "hi" ? "हिंदी" : "English"}
            </button>
        ...
        <div className="flex gap-2 mb-5">
          {(["sm", "md", "lg"] as FontSize[]).map((s) => (
            <button
              ...
              className={`flex-1 py-2 rounded-full text-sm font-serif transition-all ${
                fontSize === s ? "bg-amber-700 text-amber-100 shadow" : "bg-amber-900/60 text-amber-500 border border-amber-700/40"
              }`}
            >
              {s === "sm" ? "A" : s === "md" ? "A" : "A"}
              <span className="ml-1 text-[10px] opacity-60">{s.toUpperCase()}</span>
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2">
          <button
            onClick={() => { saveBookmark(); setOpen(false); }}
            className={`flex-1 py-2.5 rounded-full text-sm font-serif flex items-center justify-center gap-1.5 transition-all ${
              bookmarkFlash ? "bg-amber-500 text-amber-950" : "bg-amber-900/60 text-amber-400 border border-amber-700/40"
            }`}
          >
            🔖 <span>Save</span>
          </button>
          <button
            onClick={() => { goToBookmark(); setOpen(false); }}
